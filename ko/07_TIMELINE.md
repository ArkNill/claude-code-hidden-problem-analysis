...
>
> **최신 데이터 (4월 9일):** 1주일간의 프록시 데이터셋 (17,610건 요청, 129개 세션)과 업데이트된 이벤트 수는 [13_PROXY-DATA.md](../13_PROXY-DATA.md)를 참조하십시오.
...
| 지표 | 값 |
|--------|-------|
| **기간** | 14개월 (2025-02 ~ 2026-04-09, 진행 중) |
| **주요 에스컬레이션 사이클** | 9개 Phase에 걸쳐 다수 (특히 Phase 4, 6, 8, 9) |
| **최대 이슈** | #16157 — 1,422 댓글, 647 thumbs_up |
| **확인된 근본 원인** | 클라이언트 측 16건 (B1-B5, B8, B8a, B9-B11, B2a + 역사적 5건) + 서버 측 6건 (복합적) |
| **Anthropic 공식 대응** | X/Twitter (Lydia Hallie, 4월 2-3일) + GitHub #42796만 해당 (bcherny, 4월 6일, 6개 코멘트) + HN (bcherny, adaptive thinking 버그 인정). #38335/#41930/#42542 및 88개 이상의 다른 이슈에서 응답 없음 |
...
| 2026-03-24 | [**#38335**](https://github.com/anthropics/claude-code/issues/38335) | **Session limits exhausted abnormally fast since March 23** | **478** | **258** | **세 번째 메가 스레드(초대형 토론 게시글)** |
...
| 버전 | 수정 내용 | 대상 |
|---------|-----|-----------|
| v2.1.91 | `_meta["anthropic/maxResultSizeChars"]` 500K까지 확대 | 버그 5 (#10) — **MCP(외부 도구) 전용 임시 해결책** (기본 내장 도구에는 적용 안 됨) |
...
참고: 아래 번호(1-16)는 이 타임라인에서의 시간순 발견 순서입니다. 현재 추적 중인 버그에 대해, [01_BUGS.md](01_BUGS.md)에서 사용하는 B-시리즈 식별자와의 매핑은 다음과 같습니다: #1->B1, #2->B2, #8->B3, #9->B4, #10->B5, #11->B8, #12->B11, #13->B10, #14->B9, #15->B2a, #16->B8a. 항목 #3-7은 조사 과정에서 확인되었지만, 이전 버전이거나 재현이 어려운 문제이므로 별도로 추적됩니다.
...
| 10 | **Tool result budget 강제** — `applyToolResultBudget()` 기능이 도구 결과를 합산 200K로 제한 | GrowthBook 플래그 | 모두 | 한도 초과 시 도구 결과가 1-49자로 잘려 나감. **수정되지 않음** (v2.1.91에서 MCP 외부 도구만 우회 가능) |
...
### 4월 6-9일: 커뮤니티 가속 + Anthropic GitHub 응답

**#42796**이 핵심 토론장이 되었습니다 — 4월 6-8일 동안 168건의 새 코멘트가 달렸고, HN(Hacker News)에서의 바이럴 확산이 이를 증폭시켰습니다. **bcherny (Anthropic, Claude Code 리드)**가 4월 6일에만 6개의 코멘트를 작성했습니다 (HN의 바이럴 확산이 트리거였으며, 특정 코멘터에 대한 반응이 아니었습니다 — 아래 수정 사항 참조):
- `redact-thinking`은 UI 전용이며 thinking 예산에 영향을 미치지 않음을 확인
- adaptive thinking medium effort=85 기본값(3월 3일 이후)을 확인
- "이 현상을 유발할 수 있는 내부 변경 사항은 알지 못합니다"라고 언급
- 해결 방법 제안: `/effort high|max`, `CLAUDE_CODE_AUTO_COMPACT_WINDOW=400000`, `CLAUDE_CODE_SIMPLE=1`
- **HN (별도):** adaptive thinking의 zero-reasoning 버그 → 특정 턴에서 조작(fabrication) 발생을 인정. 모델팀이 조사 중. 해결 방법: `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1`
- **4월 7-8일:** 모든 이슈에서 완전한 침묵

**새로 발견된 근본 원인 (4월 6-9일):**

| # | 근본 원인 | 이슈 | 근거 | 영향 |
|---|-----------|-------|---------|--------|
| 12 | **Adaptive thinking zero-reasoning** — 특정 턴에서 추론이 전혀 출력되지 않아 조작(fabrication) 발생 | HN (bcherny) | **Anthropic 인정** | 잘못된 출력 (stripe API 버전, git SHA, apt 패키지). 해결 방법: `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING=1` |
| 13 | **TaskOutput deprecation → autocompact thrashing** — deprecation 메시지가 21배의 컨텍스트 주입을 유발 | [#44703](https://github.com/anthropics/claude-code/issues/44703) | JSONL 로그 증거, `has repro` | 3회 연속 autocompact 후 치명적 오류. 87K vs 4K 글자 |
| 14 | **`/branch` 컨텍스트 인플레이션** — branch 시 메시지 중복/압축 해제 | [#45419](https://github.com/anthropics/claude-code/issues/45419), [#40363](https://github.com/anthropics/claude-code/issues/40363) | 3건의 중복 이슈 | 한 메시지로 6%→73% 컨텍스트 |
| 15 | **SendMessage cache 완전 미스** — orchestrator SendMessage가 cache_read=0을 생성 | [#44724](https://github.com/anthropics/claude-code/issues/44724) | 수치 데이터 + cnighswonger 확인 | CLI resume(B2)와 구분됨: 시스템 프롬프트조차 캐시되지 않음 |
| 16 | **JSONL 비원자적 쓰기 → 세션 손상** — 동시 도구 실행이 tool_result 항목을 누락시킴 | [#45286](https://github.com/anthropics/claude-code/issues/45286), [#21321](https://github.com/anthropics/claude-code/issues/21321) | meta-issue에 10건 이상 중복 | 영구적 400 오류, 세션 재개 불가 |

**예비 발견 사항 (MODERATE, 검증 대기 중):**

| # | 발견 사항 | 이슈 | 상태 |
|---|---------|-------|--------|
| P1/P2 | Cache TTL 이중 티어: 1시간→5분 다운그레이드의 두 가지 트리거 — (A) 텔레메트리 비활성화, (B) 쿼터 초과 | [#45381](https://github.com/anthropics/claude-code/issues/45381) + cnighswonger 데이터 | Anthropic `has repro` (트리거 A); 관찰적 (트리거 B) |
| P3 | v2.1.64 (3월 3일) "Try the simplest approach first"를 시스템 프롬프트에 추가 | [Piebald-AI 레포](https://github.com/Piebald-AI/claude-code-system-prompts) | 변경 확인됨, 인과 관계 미증명 |
| P4 | Raw SDK 호출이 third-party 감지를 우회하여 추가 사용료 대신 플랜에 과금 | [#45380](https://github.com/anthropics/claude-code/issues/45380) | HTTP 헤더 증거, 42건 이상의 관련 이슈 |

**주요 데이터 포인트 (4월 6-9일):**
- @wpank: 47,810건 요청 추적, v2.1.63 vs v2.1.96 = 3.3배 비용 (주의: 세션 길이 불일치)
- @cnighswonger: 4,700건 호출, interceptor로 98.3% cache hit, 98%의 호출에서 비결정적 도구 순서
- @mann1x: Anthropic으로부터 170 EUR 크레딧 수령 (Max 20x, 초과 소비)
- 4월 6-9일 500건 이상 새 이슈 등록 — 지배적 주제: 토큰 소비 위기, third-party 과금 오분류, 모델 품질 저하

**수정 사항:** bcherny의 응답을 "AMD 디렉터 stellaraccident에게 응답했다"고 한 이전 설명은 사실과 달랐습니다. bcherny는 stellaraccident의 첫 코멘트보다 5시간 **앞서** 응답했습니다. 트리거는 HN의 바이럴 확산이었지, 기업 소속이 아니었습니다. 상세 내용은 [08_UPDATE-LOG.md](08_UPDATE-LOG.md)를 참조하십시오.

### 4월 10-13일 — 계속되는 침묵, 커뮤니티 자체 조직화

CC 릴리스 2건 추가 (v2.1.98, v2.1.101) — B3-B11에 대한 수정은 여전히 제로. 이 기간 동안 어떤 추적 이슈에서도 Anthropic MEMBER의 응답이 없었습니다. 유일한 준공식적 신호는 @wjordan(외부 기여자, `author_association: NONE`)이 Piebald-AI 시스템 프롬프트 아카이브를 통해 "Output efficiency" 섹션(P3)이 v2.1.98과 v2.1.101 사이 어딘가에서 시스템 프롬프트에서 제거되었다고 알린 것이었습니다.

한편 커뮤니티는 계속 도구를 만들었습니다. @cnighswonger가 fgrosswig 대시보드와 연동되는 cache-fix v1.7.1을 출시했습니다. @fgrosswig가 쿼터 제수(divisor) 분석 기능이 포함된 usage-dashboard v1.6.0을 출시했습니다. @0xNightDev (EU)가 투명 프록시를 통해 `fallback-percentage: 0.5`를 확인했고 — cnighswonger가 7일간 11,502건의 API 호출에서 분산 없이 독립적으로 검증했습니다. 이 필드의 의미는 문서화되지 않았으며; 커뮤니티 해석은 "50% 유효 용량"에서 "fallback 라우팅 비율"까지 다양하지만 — 확인된 것은 없습니다.

#42796 (325개 코멘트, 1,567개 thumbs up, 여전히 닫힌 상태)에서 구독 취소 물결과 Codex/GLM 5.1/Kimi 2.5로의 명시적 이전 보고가 이어졌습니다. 다수의 안전 사고가 보고되었습니다: 비가역적 블록체인 전송(#46947), 모델의 자체 프롬프트 인젝션 생성(#46971), Looker 대시보드 대량 삭제(#42796 코멘트). @0xNightDev가 EU 소비자 보호 문서를 준비했습니다.

**자체 데이터 (4월 13일 측정):**
- `fallback-percentage`를 **20,083건의 요청**으로 확대 (4월 4-13일): 모든 요청에서 0.5, 분산 제로
- P3 "Output efficiency"가 4월 10일 이후 모든 ~30개 세션에서 **부재** (353개 JSONL 파일 스캔)
- 첫 턴 `cache_read=0`: **79%** (3건 이상 요청이 있는 143개 세션 중 113개)
- Changelog 교차 참조를 v2.1.101까지 확장: 미수정 버그 수정 제로. B2a → POSSIBLY FIXED

---

*2026-04-02 수집, 2026-04-13 업데이트. 근본 원인 분석은 [README.md](README.md), 쿼터 아키텍처 분석은 [02_RATELIMIT-HEADERS.md](02_RATELIMIT-HEADERS.md), 일별 조사 기록은 [08_UPDATE-LOG.md](08_UPDATE-LOG.md)를 참조하십시오.*
