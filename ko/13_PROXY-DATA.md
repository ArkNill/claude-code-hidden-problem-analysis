...
> **날짜:** 2026년 4월 14일 (데이터 수집 진행 중)
>
> **데이터 소스:**
> - cc-relay 프록시 SQLite 데이터베이스 (`usage.db`, 49 MB) — 30,477건의 API 요청 가로채기 (4월 1-14일)
> - `jsonl_analyzer.py`를 이용한 JSONL 일괄 스캔 — 532개 세션 파일, 158.3 MB (4월 1-8일, 이후 날짜에 대해서는 재실행하지 않음)
>
> **다른 문서와의 관계:** [03_JSONL-ANALYSIS.md](03_JSONL-ANALYSIS.md)는 JSONL 클라이언트 측 분석 (§1-8)을 담고 있으며, 이 문서의 주요 발견사항을 참조합니다. [01_BUGS.md](01_BUGS.md)는 버그 정의를 담고 있으며, 이 문서는 전체 측정 데이터를 제공합니다. [02_RATELIMIT-HEADERS.md](02_RATELIMIT-HEADERS.md)는 서버 측 rate limit 헤더 분석을 다룹니다.
>
> **환경 변화:** 4월 10일에 프록시 기반 GrowthBook 플래그 오버라이드가 배포되었습니다. **4월 1~10일** 데이터는 수정되지 않은 환경(기준선)에서 수집된 것입니다. **4월 11~14일** 데이터는 오버라이드된 환경에서 수집된 것입니다. B4/B5 이벤트 건수는 전부 기준선 기간에서 발생한 것입니다. 상세 내용은 [01_BUGS.md](01_BUGS.md#growthbook-flag-override--controlled-elimination-test-april-1014)를 참조하십시오.
...
| 지표 | 값 (4월 1-8일) | 값 (4월 1-14일, 최신) |
|------|-----------------|--------------------------|
| 총 API 요청 수 | 17,610 | **30,477** |
| 고유 세션 수 | 129 | **230** |
| 총 input 토큰 | 12,438,471 | **28,477,426** |
| 총 output 토큰 | 8,214,875 | **15,007,647** |
| 총 cache_read | 1,692,619,956 | **2,988,290,095** |
| 총 cache_creation | 38,785,293 | **64,098,470** |
| 전체 cache % | — | **97.0%** |
| 기간 | 4월 1~8일 | **4월 1~14일** |
...
| 모델 | 요청 수 (4월 1-8일) | 요청 수 (4월 1-14일) | Cache % | 총 Cache Read | 총 Cache Create |
|------|--------------------|--------------------|---------|------------------|--------------------|
...
> **참고:** 모든 B5 이벤트는 **수정되지 않은 기준선 기간** (4월 1~10일)에서 발생한 것입니다. 4월 10일의 GrowthBook 플래그 오버라이드 이후, 4,919건의 요청에서 B5 이벤트는 0건이었습니다. 상세 내용은 [01_BUGS.md](01_BUGS.md#growthbook-flag-override--controlled-elimination-test-april-1014)를 참조하십시오.
...
| 지표 | 4월 3일만 | 4월 1-8일 | 4월 1-14일 (합계, 전부 기준선) |
|------|-----------|---------|-------------------------------|
| 총 이벤트 수 | 261 | 59,609 | **167,818** |
| 영향받은 세션 수 | 1 | ~20 | **218** |
| 잘림 비율 | 100% | 100% | **100%** |
| 오버라이드 이후 이벤트 (4월 11-14일) | — | — | **0** (4,919건 요청) |
...
> **참고:** 모든 B4 이벤트는 **수정되지 않은 기준선 기간** (4월 1~10일)에서 발생한 것입니다. GrowthBook 플래그 오버라이드 이후, 4,919건의 요청에서 B4 이벤트는 0건이었습니다.
